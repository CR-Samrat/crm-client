...
const Dashboard = () => {
    ...
    useEffect(() => {
        ...
        const loadUser = async() =>{
            await axiosInstance.get("/crm/contacts/dashboard", {
    ...
    const handleChange = (event) => {
      ...
      const handleSubmit = async(event) => {
        ...
        if(edit) {
          ...
          try {
            await axiosInstance.post(`/todo/${contact.id}/edit/${todo_id}`,todo,{
              headers: {'Content-Type' : 'application/json'}
            }).then(response => {
              setContact(response.data)
            })
          ...
          try {
            await axiosInstance.post(`/todo/${contact.id}/add`,todo,{
              headers: {'Content-Type' : 'application/json'}
            }).then(response => {
      ...
      const handleCompleteTodo = async(todoId) =>{
        try {
          await axiosInstance.get(`/todo/${contact.id}/toggle/${todoId}`)
          .then(response => {
            setContact(response.data)
          })
        } catch (error) {
      ...
      const handleDeleteTodo = async(todoId) =>{
        try {
          await axiosInstance.get(`/todo/${contact.id}/delete/${todoId}`)
          .then(response => {
            setContact(response.data)
          })
        } catch (error) {
